...
let isUserNameSameCheck = false;	// let = var
let isUserNameState = "";
...
function userJoin() {
	...
	if (!(isUserNameState == $("#userName").val())) {
		alert("아이디 중복확인 후 다시 시도해 주세요.");
		return;
	}

	let data = { // 1) 보낼 데이터를 가져오자
		userName: $("#userName").val(), // 변수명: $("#id에 적은 내용").val()
...
function userNameSameCheck() {
	...
	$.ajax(`/api/user/account/isUserNameSameCheck?userName=${userName}`, {
		type: "GET",
		dataType: "json",
		async: true
	}).done((res) => {
